# Get_Next_Line
## 開発メモおよび勉強ノート
### 1. `static t_remainder remainder`
```c
...
`static t_remainder remainder[FD_MAX]; `のように fd インデックスで配列管理を行います
...
標準的なパターンでは、
```bash
malloc でバッファを確保 # read_and_store()
↓
read(fd, buf, BUFFER_SIZE) をループ # read_and_store()
↓
改行('\n')が見つかったらそこで停止 # read_and_store()
↓
改行('\n')または 終端('\0') までを出力文字列に変換　# extract_line()
↓
以前の remainder と結合 # update_remainder()
↓
返却
```
という流れになります。

以下はコード全文とエラーハンドリングの説明メモ
...
`t_remainder remainder[MAX_FD]`とすることで、複数ファイル(のfd)に対応したポインタを用意し、GNLの引数に応じて参照するremainderを切り替えるようにした

`<unistd.h>`に _SC_OPEN_MAX という定数があるが、これを使用してテストを行うなぜかエラーとなり検証が進まなかったので、今回は自前(`# define MAX_FD 1024`)で用意することにした。~原因究明はめんどくさいのでやりません~
...
ある程度調査したところ、
普遍的なプログラム品質評価方法にいける`3秒`という制限は、以下の観点で「緩めではない」と考えています。
- CPU性能や OS スケジューリングの揺らぎ考慮としては短い
- 大量メモリ処理、巨大入力、複雑アルゴリズム向けではない
- 教育用 / 採点用の自動評価環境でよく採用される閾値

もし一般的な商用アプリケーション性能評価であるなら、ミリ秒〜数百ミリ秒単位の SLA の方が通常であり、逆に3秒は「遅くても良い」範囲のようです。ただし、本コードの文脈は 機能検証＋安全性テスト であり、速度性能テストではなく停止保証テスト なので評価軸が異なります。
...
以上のことから、デフォルト値である `TIMEOUT=3` は 「性能を測る基準ではなく、バグによる停止を迅速に検出するための実用的かつ厳しめの制限」 と評価できます。一般的な単体テストではやや短いが、教育・課題判定や CI 実装の観点としては妥当かつ保守的です。

なので、 `-tm 30`ならTOEせず動く、というのは、「異常系テストだと時間制限が厳しいと完遂できないけど、"異常があると無限ループしてしまう"ようなバグは潜んでいない」という品質をアピールすることはできる、ということになります。

補遺：
全テスト通過条件
- macOS環境(Apple M1,16GB) 						-> mandatory `-tm 180` bonus `-tm 300<`
- win/WSL環境(ADM Ryzen 7 9700X 8-Core,32GB)	-> `-tm 30`

~ラップトップだとbonus用GNLに限って5分経ってもTOEする。これは果たしてPC性能なのかコード品質のせいなのか。しかしこれに拘っているといつまで経っても次の課題に行けないのでめっちゃ悩む。~